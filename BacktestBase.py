...
class BacktestBase(object):
    ...
    def get_data(self):
        ...
        """
        raw = pd.read_csv('tickers_data.csv', index_col=0, parse_dates=True).dropna()
    ...
    def plot_data(self, cols=None):
        ...
        if cols is None:
            cols = ['price']
        self.data['price'].plot(figsize=(10, 6), title=self.symbol+' Close price')
        plt.xlabel('Date')
        plt.ylabel(cols[0])
        plt.savefig(self.symbol +' Close price' +'.png')
        plt.show()
...
if __name__ == '__main__':
    bb = BacktestBase('AAPL', '2015-1-2', '2022-05-31', 10000)
    print(bb.data.info())
    print(bb.data.tail())
    bb.plot_data()
